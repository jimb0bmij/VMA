...
ke to check!\n\n";
		...
		foreach my $host_name( @host_list ) {
			...
			if($host_view) {
				...
				print color("magenta") . "Host: " . $host_name . " ( " . $host_view->config->product->fullName . " )" . 
color("reset") . "\n";
...
sub validateConnection {
        ...
        foreach(@$licenses) {
                ...
                if($_->editionKey eq 'esxBasic' && $host_license eq 'licensed') {
                        ...
                        print "This script requires your ESX(i) be licensed, the free version will not allow you to perform any write 
operations!\n\n";
